...
class _MakeupPageState extends State<MakeupPage>
  ...
  Future<void> _processFrame(CameraImage img) async {
    ...
    try {
      ...
      final fmt = Platform.isAndroid ? InputImageFormat.nv21 : InputImageFormat.bgra8888;
      final buf = WriteBuffer();
      for (final p in img.planes) buf.putUint8List(p.bytes);
      final bytes = buf.done().buffer.asUint8List();
      final bpr = Platform.isAndroid ? img.width : img.planes.first.bytesPerRow;

      final inputImage = InputImage.fromBytes(
        bytes: bytes,
        metadata: InputImageMetadata(
          size: Size(img.width.toDouble(), img.height.toDouble()),
          rotation: rot, format: fmt, bytesPerRow: bpr,
        ),
      );
      final faces = await _detector!.processImage(inputImage);
      final rotatedSize =
          (rot == InputImageRotation.rotation90deg || rot == InputImageRotation.rotation270deg)
              ? Size(img.height.toDouble(), img.width.toDouble())
              : Size(img.width.toDouble(), img.height.toDouble());

      if (mounted) setState(() { _faces = faces; _imageSize = rotatedSize; });
    } catch (_) {
    } finally { _isDetecting = false; }
  }
    ...
    try {
      ...
      if (mounted) {
        ...
        await Navigator.push(context,
          MaterialPageRoute(builder: (_) => MakeupReportPage(
            imagePath: file.path, result: result)));
        if (!_cam!.value.isStreamingImages) await _cam!.startImageStream(_onFrame);
      }
    } catch (e) {
              ...
              child: Stack(fit: StackFit.expand, children: [
                      ...
                      if (_arEnabled && _faces.isNotEmpty)
                        CustomPaint(
                          painter: ARMakeupPainter(
                            faces: _faces,
                            imageSize: _imageSize.isEmpty ? Size(prevW, prevH) : _imageSize,
                            look: _activeLook,
                            mirrorX: true,
                ...
                if (_faces.isEmpty)
                  Positioned(
                    ...
                    child: Center(child: _Pill(text: 'Position your face to try on makeup')),
                  ),
              ]),
            ),
            _buildBottomPanel(),
      ...
      child: Container(
        ...
        child: SafeArea(child: Padding(
          ...
          child: Row(children: [
            ...
            const Expanded(
              child: Text('Makeup Try-On', textAlign: TextAlign.center,
                style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w700)),
            ),
            GestureDetector(
              onTap: () => setState(() => _arEnabled = !_arEnabled),
              child: AnimatedContainer(
                duration: const Duration(milliseconds: 300),
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                decoration: BoxDecoration(
                  color: _arEnabled ? _orchid.withOpacity(0.85) : Colors.white.withOpacity(0.15),
                  borderRadius: BorderRadius.circular(20),
                ),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(_arEnabled ? Icons.face_retouching_natural : Icons.face_retouching_off,
    ...
    return Container(
      ...
      child: Column(children: [
            ...
            itemBuilder: (_, i) {
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  width: 62,
                  decoration: BoxDecoration(
                    color: active ? _orchid.withOpacity(0.18) : Colors.white.withOpacity(0.05),
                    borderRadius: BorderRadius.circular(16),
                    border: Border.all(
                      color: active ? _orchid : Colors.white.withOpacity(0.1),
                      width: active ? 1.8 : 1.0,
                    ),
                  ),
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Text(look.emoji, style: const TextStyle(fontSize: 22)),
                    const SizedBox(height: 4),
                    Text(look.label, style: TextStyle(
                      color: active ? _orchid : Colors.white54,
                      fontSize: 10,
        ...
        Padding(
          ...
          child: Row(children: [
            const Icon(Icons.invert_colors_off, color: Colors.white38, size: 16),
            Expanded(
              child: SliderTheme(
                ...
                child: Slider(
                  value: _intensity, min: 0.0, max: 1.0,
        ...
        Padding(
          ...
          child: ScaleTransition(
            ...
            child: GestureDetector(
              ...
              onTapUp: (_) { _btnCtrl.reverse(); _capture(); },
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(color: _orchid.withOpacity(0.4), blurRadius: 18, offset: const Offset(0, 6)),
                  ],
                ),
                child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(_capturing ? Icons.hourglass_bottom_rounded : Icons.auto_fix_high_rounded,
                      color: Colors.white, size: 20),
                  const SizedBox(width: 10),
                  Text(
                    _capturing ? 'Capturing…' : 'Analyze My Features',
